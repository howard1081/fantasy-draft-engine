...
- **Playoff weighting (added same day).** The lineup objective is weekly starting-lineup points, so a 4th WR only ever counts as bench coverage while a player filling an open starting slot counts every week; roster composition is enforced by the lineup, not by position quotas. Fantasy-playoff weeks 15–17 are weighted 1.5x (`PLAYOFF_WEEKS`, `PLAYOFF_WEIGHT`). Because byes never fall in those weeks, the practical effect is to raise the value of every-week starters (an elite QB, a locked-in RB1) relative to bye-week coverage. Per-opponent playoff schedule strength is handled by the matchup layer below.

## Matchups and playoff strength of schedule — September 8, 2026

The user asked whether the draft can account for who each player faces, especially in weeks 15–17 ("a QB is not going to do as well against the Texans or the Rams"), and for the same data to feed a future weekly start/sit tool. ESPN has no stable public defense-vs-position feed, so it is derived.

- **Data.** `scripts/update-data.mjs` fetches the 2026 `proTeamSchedules_wl` feed (weekly opponent and home/away for all 32 teams) and the prior season's `kona_player_info` weekly actuals. For every player-week it attributes the player's PPR points to the defense faced; `defense[TEAM][POS]` is that defense's points allowed per game at the position divided by the league average (1.0 = average, requires 8+ games), written to `data/matchups.json` with `schedule`, `defense`, `basis` and `playoffWeeks`. Free agents (`team: FA`) are dropped from the pool because they have no schedule or bye.
- **Shrinkage, not signal.** A prior-season defense is a weak predictor of December strength (injuries, scheme, personnel turnover, weather), so `matchupFactor()` uses `1 + 0.5 * (ratio - 1)` clamped to ±10% (`MATCHUP_WEIGHT`, `MATCHUP_CAP`). Raw ratios in the 2025 data run from ~0.56 to ~1.57 for QB/RB/WR/TE and beyond 2.0 for DST; after shrinkage three quarters of player-weeks sit within ±8% and every player's full-season average is within ±3.5%. This is deliberate: the layer breaks ties between similar players and nudges playoff-week value; it must not outrank a projection edge. In opening-pick tests it flips 2 of 12 slots, each a sub-1-point decision on a ~2100-point objective.
- **Where it applies.** `rosterSeasonValue()` multiplies each starter's weekly points (and any bye-cover points) by the factor for that player's opponent that week; combined with the 1.5x weight on weeks 15–17, a tough playoff slate costs a starter roughly `1.5 × 3 × ppg × (1 - factor)`. Expected values of *future* picks stay position-level (no team is known yet), so matchups only distinguish concrete players.
- **UI.** The hero shows `Playoffs (wk 15-17): @MIN vsNYG @CHI` with a soft/neutral/tough tag (thresholds ±4% on the average playoff factor); non-neutral slates also appear as a reason chip and My Team rows carry a `PO soft/tough` tag. Missing data (older snapshot, unknown team, bye week) is neutral, so the app degrades to the previous behavior.
- **Not done.** No pass-rush/EDGE-specific model — the derived table already reflects a defense's total effect on QB scoring, which is the outcome the user cares about. In-season updates (2026 actuals replacing 2025 priors) are a future weekly-tool concern; the draft snapshot is frozen at draft time.
